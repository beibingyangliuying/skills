...
description: 当任务涉及数值实验设计、批量运行实验 case、本地 CPU 多进程实验、在运行前生成 execution plan、将可复用实验配置整理到 `experiments/*.toml`、保留实验产物供后续分析或绘图、或继续执行被中断的实验时使用。该 skill 负责实验 orchestration、结果留存、断点续跑和 plot handoff，不替代具体的数据处理或绘图规范。
---
...
用尽量轻量的约定组织数值实验，让实验在开始前可确认、运行时可追踪、结束后可定位、被中断后可继续，并为后续分析与绘图保留足够的数据和元信息。
...
- SHOULD: 若用户要继续一个被中断的实验，在 plan 中说明这是续跑已有 run，哪些 case 已完成、哪些 case 将继续。
- SHOULD: 当实验规模较大时，先对参数展开和输出目录做简要摘要，避免边解释边开跑。
...
- MUST: `run_manifest.json` 应能帮助后续定位本次 run 的基本信息、配置来源或配置摘要、case 组织方式、主要输出位置、失败记录入口和 case 状态。
- MUST: `summary.json` 应能帮助后续快速了解总体状态、成功/失败计数、关键结果摘要和剩余工作量摘要。
...
- 是否为长时间任务提供了清晰的进度展示
- 是否根据场景合理选择了串行或多进程
- 若实验被中断，后续是否能按 case 粒度继续而不是从头开始
