...
- SHOULD: 若保留旧 API、旧别名或兼容层，不仅要保持旧调用路径可用，也要明确提示用户迁移，而不是长期静默兼容。
- SEE: 需要兼容面优先级和具体实施策略时，读取 `references/compatibility-surface.md`。

### 3. 实施策略

- SHOULD: 优先做增量兼容修改，而不是直接替换旧接口。
- MUST: 若为保持向后兼容而保留旧 API、旧别名或兼容层，旧调用路径必须发出弃用 warning，不能只做静默兼容。
- SHOULD: 优先通过 `deprecated` 包实现弃用提示；若当前任务上下文不适合引入该方案，至少保留等价 warning 机制，并在结果汇报中说明原因。

### 4. 验证要求

- MUST: 修改前先识别哪些符号、导出模块、命令、配置项或文件格式已经对外暴露。
- MUST: 若保留旧 API、旧别名或兼容层，验证时不仅要检查旧调用路径仍可用，也要检查是否会发出弃用 warning。
...
- 若保留旧 API 或兼容层，是否已验证弃用 warning 会按预期发出
