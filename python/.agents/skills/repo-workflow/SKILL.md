...
- MUST: 环境确认完成后，优先自行运行最相关的最小测试集来验证修改结果。
- SHOULD: 默认优先使用 `pytest`。
- SHOULD: 若虚拟环境已经成功激活，运行测试或为用户提供 PowerShell 测试命令时，优先使用当前 shell 中的 Python 命令，例如 `python -m pytest`，不要默认切到 `uv` 入口。
...
- 是否已在环境确认完成后，自行运行最相关的最小测试集，或在需要时提供了可直接执行的 PowerShell `pytest` 命令
- 若修改触及已有 doctest，是否已尽可能保留并优先更新其内容，而不是无必要删除
- 是否已如实汇报未执行测试的原因，以及实际测试结果是否来自 AI 本次执行
